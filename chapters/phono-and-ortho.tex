...
The\marginnote{\centering\normalsize\ipa
  ...
  \begin{tabular}{ll}\toprule
    ə \\
    a \\\bottomrule
  \end{tabular}\\[0.5em]Suffix vowels} set of consonants that suffixes can have
...
shorter suffixes. As noted earlier, long \mbox{/\upa{p t t͡ʃ t͡ɕ k}/} can't be
...
suffix is added to a stem ending in \mk{-a/\"a}, there's no alterations; they
are simply glued together. However, stems that end in \mk{-e} lose the final
vowel, so this will often create illegal clusters, usually of three consonants.
For two-consonant clusters, the \mk{-e-} is usually just reinserted if the
cluster isn't allowed, as in \mk{nime-je} > \mk{nimeje} and \mk{w\"ake-ŋ\"a} >
\mk{w\"akeŋ\"a}. However, if they are two different nasals, then the second
...
In three-consonant clusters, if the first consonant is \mk{j}, then it's no
